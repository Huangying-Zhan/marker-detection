...
def vis_detections(cnt, dist, plt_cnt, FIG, im, class_name, bboxes, scores, thresh = 0.5):
	...
	for i in xrange(bboxes.shape[0]):
		...
		#                                               thresh),
		#               fontsize=10)
		ax.set_title('Picture_'+str(cnt),fontsize=14)
		plt.axis('off')
		plt.draw()
		plt.savefig("./saved_fig/fig_"+str(cnt)+".png")
		plt.pause(0.0005)
		print "PLT_CNT: ", plt_cnt

def vis_non_detections(cnt, plt_cnt, FIG, im):
	"""Draw detected bounding boxes."""
	row = 1
	col = 1
	im = im[:, :, (2, 1, 0)] #RGB -> BGR
...
def detection(net, im, CLS_IND):
	...
	for cls_ind in CLS_IND:
		...
		if len(inds) == 0:
			final_bbox.append(np.asarray([0]))
			final_scores.append(np.asarray([0]))
			continue
		final_bbox.append(dets[inds, :4])
...
rgb_img_list = sort_nicely(rgb_img_list)
depth_map_list = os.listdir(depth_map_dir)
depth_map_list = sort_nicely(depth_map_list)
	...
	while offline_mode_cnt < len(depth_map_list):
		try:
		  ...
		  # Image source mode: Offline mode (load a set of images)
			...
			# do detection
			final_bbox, final_scores = detection (net, img, CLS_IND)

			# Find out which category exist
			exist_class = []
			for i in xrange(len(final_bbox)):
				if len(final_bbox[i].shape)!=1:
					exist_class.append(i)
			...
			if exist_class!=[]:
				for class_idx in exist_class:
					# get distance of objects from depth_map
					dist = get_dist_from_depth_map (final_bbox[class_idx], depth_map, img_shape)
					for i in dist:
						thefile.write("%s" % i)
					thefile.write("\n")
					...
					if MODE == 1:
						class_name = CLASSES[CLS_IND[class_idx]]
						timer.tic()
						vis_detections(offline_mode_cnt, dist, plt_cnt, FIG, img, class_name, final_bbox[class_idx], final_scores[class_idx], CONF_THRESH)
						timer.toc()
				...
				timer.tic()
				vis_non_detections(offline_mode_cnt, plt_cnt, FIG, img)
				timer.toc()
				print ('Detection took {:.3f}s for visualization').format(timer.diff)
				plt_cnt += 1
				if plt_cnt == 2:
